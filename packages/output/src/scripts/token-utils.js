...
const TOKENS_PATH = resolve(__dirname, '../../../tokens/src/tokens.json');
const HEX_TOKENS_PATH = resolve(
  __dirname,
  '../../../tokens/data/resolved-hexes.json'
);
...
export function loadHexTokens() {
  const tokens = JSON.parse(readFileSync(HEX_TOKENS_PATH, 'utf-8'));
  return tokens;
}
  ...
  for (const [key, val] of Object.entries(obj)) {
    ...
    if (val && typeof val === 'object' && 'value' in val) {
      out.push({
        ...
        extensions: val.$extensions || null // Include extensions with modifiers
      });
      continue;
    }
...
function resolveReferenceValue(refPath, setData, globalTokenIndex, depth) {
  ...
  if (globalTokenIndex.has(refPath)) {
    const tokenData = globalTokenIndex.get(refPath);
    // Handle both old format (string value) and new format (object with value + extensions)
    const value = typeof tokenData === 'string' ? tokenData : tokenData.value;
    return resolveTokenValue(value, setData, globalTokenIndex, depth + 1);
  }
...
/**
 * Get the full token object (including extensions/modifiers) from the index
 */
export function getTokenFromIndex(refPath, globalTokenIndex) {
  if (globalTokenIndex.has(refPath)) {
    const tokenData = globalTokenIndex.get(refPath);
    return typeof tokenData === 'string'
      ? { value: tokenData, extensions: null }
      : tokenData;
  }
  return null;
}
...
export function createGlobalTokenIndex(tokens) {
  ...
  for (const [setName, setData] of getTokenSets(tokens)) {
    const flattened = flattenTokenObject(setData);
    for (const token of flattened) {
      // Store full token object (with extensions/modifiers), not just the value
      const tokenObj = {
        value: token.value,
      ...
      if (!index.has(token.path)) {
        index.set(token.path, tokenObj);
      }

      // Fully-qualified path key for deterministic addressing.
      index.set(`${setName}.${token.path}`, tokenObj);
    }
  }
