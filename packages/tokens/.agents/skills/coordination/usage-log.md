...
* Feature slugs are the `.design/<feature>/` slugs already visible in the repo. If a feature name is itself sensitive (an unreleased content project), use a neutral slug for both the folder and the log; the mapping lives with the humans, not in the file.
