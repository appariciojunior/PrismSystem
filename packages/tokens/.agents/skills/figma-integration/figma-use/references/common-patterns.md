...
> Part of the [figma-use skill](../SKILL.md). Working code examples for frequently used operations.
...
// Layout
const colW = 140
componentSet.children.forEach((child, i) => { child.x = i * colW; child.y = 0 })
let maxX = 0, maxY = 0
for (const c of componentSet.children) {
  maxX = Math.max(maxX, c.x + c.width)
  maxY = Math.max(maxY, c.y + c.height)
...
For component sets with many variants (50+), split into multiple script calls.
...
// CRITICAL: layout variants in a structured grid
