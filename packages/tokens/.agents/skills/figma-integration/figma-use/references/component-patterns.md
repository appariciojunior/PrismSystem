...
> Part of the [figma-use skill](../SKILL.md). How to correctly use the Plugin API for components, variants, and component properties.
>
> For design system context (when to use variants vs properties, code-to-Figma translation, property model), see [working-with-design-systems/wwds-components.md](working-with-design-systems/wwds-components.md) (if available).
...
Variant names use a `Property=Value` format. Every unique combination must exist as a child component.
...
After `combineAsVariants`, all children stack at `(0, 0)`. You **must** position them or the component set will appear as a single collapsed element.
...
When a component has many possible sub-elements (e.g., 30 different icons), **never** create a variant per sub-element. Use a single INSTANCE_SWAP property instead.
...
**Always inspect the file before creating components.** Match existing naming styles, structures, and conventions.
...
`importComponentByKeyAsync` and `importComponentSetByKeyAsync` import components from **team libraries** (not the same file you're working in). For components in the current file, use `figma.getNodeByIdAsync()` or `findOne()`/`findAll()`.
...
**Always discover component properties BEFORE writing text overrides.** Use `setProperties()` for TEXT-type properties — direct `node.characters` changes on property-managed text may be overridden.
...
**Step 3: Only fall back to direct node.characters for unmanaged text.** Always load the node's actual font first:
...
When `detachInstance()` is called on a nested instance, the parent instance may also get implicitly detached (new ID). Subsequent `getNodeByIdAsync(oldParentId)` returns null.
...
If you must detach multiple nested instances across sibling components, do it in a **single** call — discover all targets by traversal at the start before any detachment mutates the tree.
 ...
 * otherwise returns the component itself.
 */
...
async function getLocalComponentMetadata(componentId) {
  ...
  if (node.type === "COMPONENT_SET" || node.type === "COMPONENT") {
    const result = { name: node.name, nodeId: node.id, properties: {}, descendants: {} };
