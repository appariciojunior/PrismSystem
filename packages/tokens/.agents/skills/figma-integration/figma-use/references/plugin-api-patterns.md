...
> Part of the [figma-use skill](../SKILL.md). Quick reference for common Figma Plugin API operations.
...
Page context resets between calls — `figma.currentPage` always starts on the first page. Use `await figma.setCurrentPageAsync(page)` at the start of each invocation to switch to the correct page.
...
Scripts are automatically wrapped in an async IIFE with error handling. Just write plain JS and use `return` to send data back:
...
Verify structure with `get_metadata` between steps. Use `get_screenshot` after each major creation milestone.
