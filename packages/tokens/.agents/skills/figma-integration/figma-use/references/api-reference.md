...
> Part of the [figma-use skill](../SKILL.md). What works and what doesn't in the `figma_execute` / `use_figma` environment.
...
figma.createSlice()
figma.createPage()              // Page node; child persistence is limited in headless mode
figma.createSection()
...
figma.combineAsVariants(components, parent?)   // Combine ComponentNodes into ComponentSet
```
...
These methods import components from **team libraries** (not the same file you're working in). For components in the current file, use `figma.getNodeByIdAsync()` or `findOne()`/`findAll()`.
...
These methods import styles from **team libraries** (not the same file).
...
node.setExplicitVariableModeForCollection(collection, modeId)  // pass collection object
...
```js
// Fills & Strokes (read-only arrays — must clone to modify)
...
figma.createComponentFromNode(node) // Convert existing node to component
```
...
| `TextStyle.setBoundVariable()` | Not available in headless — use raw values |
| `getPluginData()` / `setPluginData()` | Not supported — use `getSharedPluginData()` |
| `console.log()` as output | Not returned to agent — use `return` |
