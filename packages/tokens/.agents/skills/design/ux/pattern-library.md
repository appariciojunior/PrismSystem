...
**Anatomy.** Global display (product chrome, not redesigned per feature) · section navigation · in-page anchors for long reads · a persistent way back (experience-principle 5). Breadcrumbs only where the hierarchy is genuinely deep.
...
**Anatomy.** A repeating card: optional `Image` (fixed ratio, usually 3:2 or 16:9) · optional `Label` (section) · headline (`brand-heading-*`) · optional intro (`brand-intro-*`) · meta/timestamp (`brand-meta-*`) · optional `Flag` (LIVE, UPDATED). Lists compose cards on the grid (`ux/page-templates`).
**Content rules.** One headline per card. Intro is optional and short. Timestamps are relative for recent, absolute beyond a day. No clickbait — brand headlines are direct.
...
**Accessibility.** The whole card is one link with an accessible name from the headline; nested links (meta author) are separate, not overlapping targets. Image alt from the caption or marked decorative.
...
**Anatomy.** `Label` (section) → headline (`brand-heading-fluid-*`) → intro → meta + timestamp → hero `Image` → body (`brand-paragraph-*`, 60–75 char measure) → inline `Link`s (brand sentiment) → callouts, inline `Flag`s → related/UpNext rail → comments (or `CommentsDisabled`).
