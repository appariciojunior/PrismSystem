...
3. **Say light or dark** if you know it.
4. **Attach the screen.** A screenshot is fine for most things. For colour and token precision, open the file in the Figma desktop app and say "it's my active tab".
5. **Say what you want back.** A full review, just accessibility, just the copy, a spec for engineering.
6. One screen per image, full frame, sharp. Crop to the screen, not a corner of it.
7. If detail matters (a button state, a tooltip), send a second close-up image and say which is which.
8. You can always answer follow-up questions later. A rough prompt beats no prompt.
9. When in doubt, type `/design-start` and click your way through.
...
```
/design critique the attached screen. It's an article page, dark theme. Full review please.
```
...
```
/design critique my active Figma tab. It's the homepage. Check the tokens too.
```
...
**Avoid:** sending a bare image with just "critique this". Without knowing what the screen is, the feedback has to hedge.
...
**What you get back:** around six quick, rough versions, each from a different angle: denser and more content, image-led, calmer and more open, bolder headline, mobile-first, a different type treatment. They are lined up so you can compare, pick one, or mix two. Only the one you choose gets built properly afterwards.
...
You are not starting from scratch. You have a real screen and want to add a feature or change part of it, keeping everything else intact.
...
```
/design take this screen and swap the hero for a live-blog module. <screenshot attached>
```

```
/design reuse this layout for the new releases page. Same structure, same styling.
```

**What you get back:** the updated screen, built from the same grid and components as the original, plus a short note listing exactly what changed. With Figma as your active tab the system reads the real tokens; with a screenshot it matches what it can see.
...
```
/design design a live-blog experience. Web first, mobile matters.
```

```
/design we need an onboarding flow for the mobile app. Three steps max. Help me shape it.
```

```
/design what's the house pattern for a paywall on a long read? Then design one.
```
...
**Avoid:** skipping the context. "Design a settings page" gets a competent guess; "design a settings page for the app, our users are mostly over 50" gets an on-brand answer.
...
Something is not quite right: a blue that is nearly the brand blue, or a dark mode that drifts. This is a token question, and it needs Figma.

```
/design my active Figma tab: check every colour is bound to the right token.
```
...
```
/design is this blue using the right token, or is it hard-coded somewhere?
```
...
```
/design what's the rule on colour tokens in components?
```
...
**Good:** `/design critique the attached article screen. Dark theme. Full review please.`
The good one names the screen and the theme, so every check runs against the right rules instead of hedging.
...
**Good:** `/design build on my active Figma tab: add a share button to the article footer. Keep everything else as it is.`
...
No. A screenshot is enough for layout, hierarchy, UX, copy and a rough colour read, which covers most everyday questions. You only need Figma when the answer depends on what a value is actually bound to: token audits, light versus dark comparisons, and handoffs.

**What if I only have a screenshot?**
Paste it and carry on. Say what the screen is, and you will get a full, useful review. The system will tell you if something you asked for genuinely needs Figma.
...
Two fixes. First, add context: what the screen is and the theme. Second, teach the system from real screens (recipe 9); once it has studied the shipped product, its advice cites how your brand actually designs. If you called something very specific directly and it feels flat, go back through `/design` instead, which always loads the brand foundations first.
