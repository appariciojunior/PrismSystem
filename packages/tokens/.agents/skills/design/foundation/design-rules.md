...
| DS-TYP-03 | Body text uses a `brand.paragraph.*` or `utility.body.*` token, not a caption or meta token in a body role | warning | typography system | design-critique A |
| DS-TYP-04 | Type comes from one system; no mixing legacy `content*` with `brand.*`/`utility.*` in one frame | error | token architecture; Home page v2 finding | token-mapping-audit |
...
| DS-SPC-05 | Content rhythm follows the type scale, not the grid alone | info | DNA principle 3 | design-critique B |
...
| DS-MOT-06 | No hero scroll reveals or bouncy mascot motion on content surfaces | warning | DNA anti-patterns | motion-review |
