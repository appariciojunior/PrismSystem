...
Flag highlights short contextual metadata such as LIVE, UPDATED, or section emphasis alongside article information. Use Flag for compact status or editorial signals, not as a clickable control.

## Properties

| Figma Property | Code Property | Type    | Allowed Values                    | Default   | Description                    |
| -------------- | ------------- | ------- | --------------------------------- | --------- | ------------------------------ |
| `intent`       | `intent`      | enum    | `primary`, `secondary`, `callout` | `primary` | Changes visual treatment.      |
...
- Use for short editorial signals beside article metadata.
...
```js
// Storybook Args
{ intent: 'callout', size: 'small', iconLeft: true, iconRight: true, label: 'LIVE' }
```
...
| Contrast         | Text and background pairing must preserve readable contrast per intent.                | Validate primary, secondary, and callout variants in both light and dark story modes. |
