#!/usr/bin/env python3
"""Helper to write the new generate-figma-make-css.js. Delete after use."""
import os

TARGET = os.path.join(os.path.dirname(__file__), 'generate-figma-make-css.js')
...
// ============================================================================
// Ramp Color Extraction (from CSV + palette token sets)
// ============================================================================

function loadRampCSV(csvPath) {
  ...
  try {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      const [mode, ramp, step, hex] = parts;
      const modeKey = mode.toLowerCase().trim();
      if (!lookup[modeKey]) lookup[modeKey] = {};
      if (!lookup[modeKey][ramp]) lookup[modeKey][ramp] = {};
      lookup[modeKey][ramp][step.trim()] = hex.trim();
    }
  } catch (e) {
    console.warn('  Warning: Could not load CSV:', e.message);
  }
  return lookup;
}
...
function buildRampLookup(tokens, csvPath) {
  ...
  const digitalBlueLight = {
    '50': '#e6f3f9', '100': '#cce7f3', '150': '#b3dbec', '200': '#99cfe6',
    '250': '#80c3df', '300': '#66b7d9', '350': '#4dabd2', '400': '#339fcc',
    '450': '#1a93c5', '500': '#0087bf', '550': '#007bb0', '600': '#006fa1',
    '650': '#006392', '700': '#005783', '750': '#004b74', '800': '#003f65',
    '850': '#003356', '900': '#002747', '950': '#001b38', '1000': '#000f29'
  };
  const digitalBlueDark = {
    '50': '#000f29', '100': '#001b38', '150': '#002747', '200': '#003356',
    '250': '#003f65', '300': '#004b74', '350': '#005783', '400': '#006392',
    '450': '#006fa1', '500': '#007bb0', '550': '#0087bf', '600': '#1a93c5',
    '650': '#339fcc', '700': '#4dabd2', '750': '#66b7d9', '800': '#80c3df',
    '850': '#99cfe6', '900': '#b3dbec', '950': '#cce7f3', '1000': '#e6f3f9'
  };
  lookup.light['digital.blue'] = { ...digitalBlueLight, ...(lookup.light['digital.blue'] || {}) };
  lookup.dark['digital.blue'] = { ...digitalBlueDark, ...(lookup.dark['digital.blue'] || {}) };

  const digitalCreamLight = {
    '50': '#fefdfb', '100': '#fcfaf5', '150': '#faf8f0', '200': '#f8f5ea',
    '250': '#f6f3e5', '300': '#f4f0df', '350': '#f2eeda', '400': '#f0ebd4',
    '450': '#eee9cf', '500': '#ece6c9', '550': '#e4ddc0', '600': '#dcd4b7',
    '650': '#d4cbae', '700': '#ccc2a5', '750': '#c4b99c', '800': '#bcb093',
    '850': '#b4a78a', '900': '#ac9e81', '950': '#a49578', '1000': '#9c8c6f'
  };
  const digitalCreamDark = {
    '50': '#9c8c6f', '100': '#a49578', '150': '#ac9e81', '200': '#b4a78a',
    '250': '#bcb093', '300': '#c4b99c', '350': '#ccc2a5', '400': '#d4cbae',
    '450': '#dcd4b7', '500': '#e4ddc0', '550': '#ece6c9', '600': '#eee9cf',
    '650': '#f0ebd4', '700': '#f2eeda', '750': '#f4f0df', '800': '#f6f3e5',
    '850': '#f8f5ea', '900': '#faf8f0', '950': '#fcfaf5', '1000': '#fefdfb'
  };
  lookup.light['digital.cream'] = { ...digitalCreamLight, ...(lookup.light['digital.cream'] || {}) };
  lookup.dark['digital.cream'] = { ...digitalCreamDark, ...(lookup.dark['digital.cream'] || {}) };

  // Messaging ramps
  const messagingRamps = {
    'messaging.error': {
      light: {
        '50': '#ffe5e5', '100': '#ffcccc', '150': '#ffb2b2', '200': '#ff9999',
        '250': '#ff7f7f', '300': '#ff6666', '350': '#ff4c4c', '400': '#ff3333',
        '450': '#ff1919', '500': '#ff0000', '550': '#e60000', '600': '#cc0000',
        '650': '#b30000', '700': '#990000', '750': '#800000', '800': '#660000',
        '850': '#4d0000', '900': '#330000', '950': '#1a0000', '1000': '#0d0000'
      },
      dark: {
        '50': '#0d0000', '100': '#1a0000', '150': '#330000', '200': '#4d0000',
        '250': '#660000', '300': '#800000', '350': '#990000', '400': '#b30000',
        '450': '#cc0000', '500': '#e60000', '550': '#ff0000', '600': '#ff1919',
        '650': '#ff3333', '700': '#ff4c4c', '750': '#ff6666', '800': '#ff7f7f',
        '850': '#ff9999', '900': '#ffb2b2', '950': '#ffcccc', '1000': '#ffe5e5'
      }
    },
    'messaging.success': {
      light: {
        '50': '#e5f7ed', '100': '#cbf0db', '150': '#b7ead3', '200': '#9de3c1',
        '250': '#83dcaf', '300': '#69d59d', '350': '#4fce8b', '400': '#3bc87d',
        '450': '#31b870', '500': '#31A46F', '550': '#2d9765', '600': '#2a8f60',
        '650': '#237a51', '700': '#1c6542', '750': '#195a3b', '800': '#155033',
        '850': '#0e3b24', '900': '#0a2b1a', '950': '#061b10', '1000': '#030d08'
      },
      dark: {
        '50': '#030d08', '100': '#061b10', '150': '#0a2b1a', '200': '#0e3b24',
        '250': '#155033', '300': '#195a3b', '350': '#1c6542', '400': '#237a51',
        '450': '#2a8f60', '500': '#2d9765', '550': '#31A46F', '600': '#3bc87d',
        '650': '#4fce8b', '700': '#63c297', '750': '#83dcaf', '800': '#9de3c1',
        '850': '#b7ead3', '900': '#cbf0db', '950': '#e5f7ed', '1000': '#f2fbf6'
      }
    },
    'messaging.warning': {
      light: {
        '50': '#fff5e5', '100': '#ffeccc', '150': '#ffe2b2', '200': '#ffd999',
        '250': '#ffcf7f', '300': '#ffc366', '350': '#ffba4c', '400': '#ffb333',
        '450': '#ffa919', '500': '#ffa300', '550': '#f09700', '600': '#e69300',
        '650': '#cc8300', '700': '#b37200', '750': '#996200', '800': '#805200',
        '850': '#664100', '900': '#4d3100', '950': '#332100', '1000': '#1a1000'
      },
      dark: {
        '50': '#1a1000', '100': '#332100', '150': '#4d3100', '200': '#664100',
        '250': '#805200', '300': '#996200', '350': '#b37200', '400': '#cc8300',
        '450': '#e69300', '500': '#f09700', '550': '#ffa300', '600': '#ffa919',
        '650': '#ffb333', '700': '#ffc366', '750': '#ffcf7f', '800': '#ffd999',
        '850': '#ffe2b2', '900': '#ffeccc', '950': '#fff5e5', '1000': '#fffaf2'
      }
    },
    'messaging.info': {
      light: {
        '50': '#e5eef8', '100': '#ccddf1', '150': '#b2cceb', '200': '#99bbe4',
        '250': '#7faade', '300': '#6699d7', '350': '#4c88d1', '400': '#3377ca',
        '450': '#2161b6', '500': '#0F4AA2', '550': '#0d4092', '600': '#0b3682',
        '650': '#0a2c72', '700': '#082862', '750': '#061e52', '800': '#051a48',
        '850': '#041442', '900': '#030f32', '950': '#020a22', '1000': '#010512'
      },
      dark: {
        '50': '#010512', '100': '#020a22', '150': '#030f32', '200': '#041442',
        '250': '#051a48', '300': '#061e52', '350': '#082862', '400': '#0a2c72',
        '450': '#0b3682', '500': '#0d4092', '550': '#0F4AA2', '600': '#2161b6',
        '650': '#3377ca', '700': '#4f7ac2', '750': '#6699d7', '800': '#7faade',
        '850': '#99bbe4', '900': '#b2cceb', '950': '#ccddf1', '1000': '#e5eef8'
      }
    }
  };
  for (const [rampName, modes] of Object.entries(messagingRamps)) {
    lookup.light[rampName] = { ...modes.light, ...(lookup.light[rampName] || {}) };
  ...
  for (const mode of ['light', 'dark']) {
    ...
    for (let step = 50; step <= 1000; step += 50) {
      ...
      lookup[mode]['digital.overlay.dark'][String(step)] = `rgba(0, 0, 0, ${alpha})`;
      lookup[mode]['digital.overlay.light'][String(step)] = `rgba(255, 255, 255, ${alpha})`;
    }
  }

  const lightRampCount = Object.keys(lookup.light).length;
  const darkRampCount = Object.keys(lookup.dark).length;
  console.log(`   Loaded ramps: ${lightRampCount} light, ${darkRampCount} dark`);
  return lookup;
}
...
function buildFoundationLookup(tokens) {
  ...
  function traverse(obj, pathArr = []) {
    for (const key in obj) {
      ...
      const val = obj[key];
      if (val && typeof val === 'object') {
        if (val.value !== undefined && (val.type === 'color' || typeof val.value === 'string')) {
...
function resolveValue(ref, mode, rampColors, foundationLookup, semanticLookup, depth) {
  ...
  const coreRampMatch = refPath.match(/^brand\.core\.ramp\.(.+)\.(\d+(?:-base)?)$/);
  if (coreRampMatch) {
    let [, rampPath, step] = coreRampMatch;
    step = step.replace('-base', '');
    if (rampColors[modeKey]?.[rampPath]?.[step]) return rampColors[modeKey][rampPath][step];
    const parts = rampPath.split('.');
    for (let i = parts.length; i > 0; i--) {
      const tryKey = parts.slice(0, i).join('.');
      if (rampColors[modeKey]?.[tryKey]?.[step]) return rampColors[modeKey][tryKey][step];
    }
  }

  // 2. brand.channels.ramp.{channel}.{step}
  const channelRampMatch = refPath.match(/^brand\.channels\.ramp\.(\w+)\.(\d+(?:-base)?)$/);
  if (channelRampMatch) {
    let [, channel, step] = channelRampMatch;
    step = step.replace('-base', '');
    if (rampColors[modeKey]?.[channel]?.[step]) return rampColors[modeKey][channel][step];
  }

  // 3. Overlay ramps
  const overlayMatch = refPath.match(/^brand\.core\.ramp\.digital\.overlay\.(light|dark)\.(\d+)$/);
  if (overlayMatch) {
    const [, variant, step] = overlayMatch;
    const alpha = (parseInt(step) / 1000).toFixed(2);
    return variant === 'dark' ? `rgba(0, 0, 0, ${alpha})` : `rgba(255, 255, 255, ${alpha})`;
  }
  ...
  if (semanticLookup?.[refPath]) {
    ...
    if (typeof resolved === 'string' && resolved.startsWith('{')) {
      return resolveValue(resolved, mode, rampColors, foundationLookup, semanticLookup, depth + 1);
    }
  ...
  if (foundationLookup['foundation.' + refPath]) return foundationLookup['foundation.' + refPath];
...
function processSemanticTokens(tokens, setName, mode, rampColors, foundationLookup) {
  const tokenSet = tokens[setName];
  if (!tokenSet) { console.error(`Token set "${setName}" not found`); return {}; }
  ...
  const semanticLookup = { ...rawTokens };
  const result = {};

  for (const [tokenPath, rawValue] of Object.entries(rawTokens)) {
    const cssName = tokenPath.toLowerCase().replace(/[^a-z0-9]/g, '-').replace(/-+/g, '-');
    let resolvedValue;

    if (typeof rawValue === 'object' && rawValue !== null) {
      if (rawValue.x !== undefined || rawValue.y !== undefined) {
        const s = rawValue;
        const color = (typeof s.color === 'string' && s.color.startsWith('{'))
          ? resolveValue(s.color, mode, rampColors, foundationLookup, semanticLookup, 0)
      ...
      resolvedValue = resolveValue(rawValue, mode, rampColors, foundationLookup, semanticLookup, 0);
    } else {
...
function extractTypographyTokens(tokens) {
  ...
  if (fnd) {
    ...
    // Line heights
    const lineHeights = {
      'line-height-100': '100%', 'line-height-112': '112.5%',
      'line-height-125': '125%', 'line-height-150': '150%',
      'line-height-175': '175%', 'line-height-200': '200%',
    };
...
function extractSpacingTokens(tokens) {
  ...
  if (vs.spacing?.static) {
    for (const [key, val] of Object.entries(vs.spacing.static)) {
      if (key.startsWith('$')) continue;
      if (val?.value !== undefined) result[`spacing-static-${key}`] = `${val.value}px`;
    }
  }
  ...
  if (vs.spacing?.fluid) {
    for (const [key, val] of Object.entries(vs.spacing.fluid)) {
      ...
      if (val?.value !== undefined) {
        ...
        result[`spacing-fluid-${key}`] = typeof value === 'string' && value.endsWith('px') ? value : `${value}px`;
      }
    }
  }
  ...
  if (br) {
    for (const [brKey, brVal] of Object.entries(br)) {
      ...
      if (brVal?.value !== undefined) {
        ...
        result[brKey === 'full' ? 'border-radius-full' : `border-radius-${brKey}`] = `${brValue}px`;
      }
    }
  }
  ...
  if (fnd?.grid) {
    ...
    if (g.gutter?.small?.value) result['grid-gutter'] = `${g.gutter.small.value}px`;
    if (g.margin?.small?.value) result['grid-margin'] = `${g.margin.small.value}px`;
  }
  ...
  for (const direction of ['down', 'up']) {
    ...
    for (const [key, entry] of Object.entries(shadows)) {
      ...
      if (typeof s === 'object') {
        ...
        if (typeof color === 'string') color = color.replace(/\{brand\.black\}/g, '0, 0, 0');
        result[`shadow-${direction}-${key}`] = `${s.x||0}px ${s.y||0}px ${s.blur||0}px ${s.spread||0}px ${color}`;
      }
    }
  }
...
function extractBreakpointTokens(tokens) {
  const result = {};
...
function groupTokensByCategory(tokens) {
  const categories = {};
  const catOrder = [
    'Surfaces', 'Text', 'Icons', 'Borders', 'Input', 'Feedback',
    'Interactive', 'Selection', 'Selected', 'Active', 'Focus', 'Tags', 'Other'
  ];
  ...
  const ordered = {};
  for (const cat of catOrder) { if (categories[cat]) ordered[cat] = categories[cat]; }
  for (const cat in categories) { if (!ordered[cat]) ordered[cat] = categories[cat]; }
  return ordered;
}

function generateCSS(lightTokens, darkTokens, typographyTokens, spacingTokens, shadowTokens, breakpointTokens) {
  ...
  for (const [cat, toks] of Object.entries(groupTokensByCategory(lightTokens))) {
    css += `\n  /* ${cat} */\n`;
  ...
  const typoCats = {
    'Font Families': {}, 'Font Sizes (Small Viewport / Mobile)': {},
  ...
  for (const [tn, tv] of Object.entries(typographyTokens)) {
    ...
    else if (tn.startsWith('font-size')) typoCats['Font Sizes (Small Viewport / Mobile)'][tn] = tv;
  ...
  const spCats = {
    ...
    'Border Radius': {}, 'Grid (Small Viewport)': {},
  };
  for (const [sn, sv] of Object.entries(spacingTokens)) {
    if (sn.startsWith('spacing-static')) spCats['Static Spacing (fixed across viewports)'][sn] = sv;
    else if (sn.startsWith('spacing-fluid')) spCats['Fluid Spacing (scales with viewport multiplier)'][sn] = sv;
  ...
  const shCats = {
    'Foundation Shadows - Down': {}, 'Foundation Shadows - Up': {},
    'Semantic Elevation': {},
  };
  for (const [shn, shv] of Object.entries(shadowTokens)) {
    if (shn.startsWith('shadow-down-')) shCats['Foundation Shadows - Down'][shn] = shv;
    else if (shn.startsWith('shadow-up-')) shCats['Foundation Shadows - Up'][shn] = shv;
    else if (shn.startsWith('shadow-elevation')) shCats['Semantic Elevation'][shn] = shv;
  }
  ...
  css += `   ============================================================================ */\n`;
  css += `:root {\n`;
  for (const [n, v] of Object.entries(breakpointTokens)) css += `  --${n}: ${v};\n`;
  css += `}\n`;
...
async function main() {
  ...
  const rampColors = buildRampLookup(tokens, RAMP_CSV_PATH);
  ...
  const lightTokens = processSemanticTokens(tokens, LIGHT_SET, 'light', rampColors, foundationLookup);
  console.log(`   Found ${Object.keys(lightTokens).length} colour tokens`);

  console.log('Processing dark mode colour tokens...');
  const darkTokens = processSemanticTokens(tokens, DARK_SET, 'dark', rampColors, foundationLookup);
  ...
  console.log(`   Found ${Object.keys(typographyTokens).length} typography tokens`);

  console.log('Extracting spacing & layout tokens...');
  const spacingTokens = extractSpacingTokens(tokens);
  console.log(`   Found ${Object.keys(spacingTokens).length} spacing/layout tokens`);
  ...
  console.log(`   Found ${Object.keys(breakpointTokens).length} breakpoint tokens`);

  // Generate CSS
  console.log('\nGenerating globals.css...');
  const css = generateCSS(lightTokens, darkTokens, typographyTokens, spacingTokens, shadowTokens, breakpointTokens);
  ...
  if (fs.existsSync(assetsStylesDir)) {
    ...
    fs.writeFileSync(path.join(assetsStylesDir, 'globals-bridged.css'), bridgedCss);
    console.log('   Also updated: assets/styles/');
  }

  // Summary
  const totalTokens =
    Object.keys(lightTokens).length + Object.keys(darkTokens).length +
    Object.keys(typographyTokens).length + Object.keys(spacingTokens).length +
    Object.keys(shadowTokens).length + Object.keys(breakpointTokens).length;

  console.log('\nGeneration complete!');
  console.log(`   Light mode colours: ${Object.keys(lightTokens).length} variables`);
  console.log(`   Dark mode colours:  ${Object.keys(darkTokens).length} variables`);
  console.log(`   Typography:         ${Object.keys(typographyTokens).length} variables`);
  console.log(`   Spacing/Layout:     ${Object.keys(spacingTokens).length} variables`);
  console.log(`   Shadows:            ${Object.keys(shadowTokens).length} variables`);
  console.log(`   Breakpoints:        ${Object.keys(breakpointTokens).length} variables`);
